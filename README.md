...
- **後續步驟規劃**：
  ...
  4. [x] **整合路由與環境變數**:
      ...
      *   設定 `JWT_SECRET` 等環境變數。
  5. [x] **前端頁面連接**:
      *   [x] 修改註冊/登入頁面以呼叫後端 API。
      *   [x] 處理前端 JWT 儲存與登入狀態管理 (使用 `localStorage` 和 `AuthContext`)。
      *   [x] 實作登出功能。
      *   [x] 實作路由保護 (儀表板頁面)。
      *   [x] 建立前端 API 工具函數 (`lib/api.ts`)。
- **預計完成時間**：第 3-4 週。
### 特殊任務 1： 前端頁面翻譯成中文並加入切換語系的功能（已完成）

1. **目標說明**  
   - 將前端專案資料夾中的所有英文文字翻譯成繁體中文，並確保用詞符合台灣在地習慣和語境。

2. **任務拆解**  
   - A. 收集與整理需要翻譯的英文文字  
     - 1) 搜尋所有前端檔案（如 .html、.js、.jsx、.vue、.json等），找出硬編碼 (hard-coded) 的英文字串。  
     - 2) 若已經有使用 i18n，請定位所有 key-value 並匯出待翻譯的內容。  
     - 3) 若尚未使用 i18n，暫時把所有需要翻譯的字串整理成文件/試算表，以便後續做批次翻譯。

   - B. 引入或設定多語系架構（如需要）  
     - 1) 規劃專案結構：在 `locales` 或類似資料夾中建立 `en.json`、`zh-TW.json`。  
     - 2) 若專案使用 React、Vue 或 Angular 等框架，導入對應的 i18n 套件（例如 react-i18next、vue-i18n、ngx-translate）。  
     - 3) 修改前端程式碼，將硬編碼字串替換成對應的多語系 key。

   - C. 翻譯與用詞校正  
     - 1) 使用 ChatGPT 或其他翻譯工具將英文字串翻譯成繁體中文。  
     - 2) 重點檢查台灣常用語：如「帳號」、「手機」、「訂單」、「顧客」等，而不是「賬戶」、「手機號」、「訂單號」、「客戶」。  
     - 3) 若有專業術語或客製用詞，建立並維護詞彙表，確保翻譯一致。

   - D. 整合與測試  
     - 1) 將翻譯完成的字串整合回 i18n 檔案（或其他翻譯檔）。  
     - 2) 將系統切換到繁體中文模式，檢查介面字詞是否顯示正確及符合語境。  
     - 3) 進行裝置相容測試：桌機、平板、手機等，避免跑版或多餘換行。  
     - 4) 若有疑慮字詞，與團隊或台灣在地使用者再次討論修正。
   - E. 實作切換語系功能
     - 1) 在前端介面新增語系切換按鈕（例如「English / 繁體中文」），並與 i18n 或翻譯檔案做互動。
     - 2) 在專案程式碼中導入多語系邏輯（例如 react-i18next、vue-i18n 或其他 i18n 工具）。
     - 3) 設定預設語系（通常可偵測瀏覽器語系，或預設為繁體中文），並提供使用者手動切換語系的選項。
     - 4) 測試切換流程：確保從英文切換到繁體中文，或繁體中文切換到英文時，所有介面文字皆正確顯示。
     - 5) 若有需儲存使用者語系偏好的需求，可透過 Cookie 或 Local Storage 進行紀錄，避免每次重新載入都回到預設語系。

3. **交付成果**  
   - 整理後的繁體中文翻譯檔案（例如 `zh-TW.json` 或對應翻譯文件）。  
   - 翻譯詞彙表或專有名詞對照表（如有）。  
   - 確認在專案中能正常切換語系，且文字呈現自然、流暢。

4. **後續維護**  
   - 建立或更新翻譯流程文件：新功能推出時，如何追蹤新增/變動的字串，確保翻譯即時更新。  
   - 由版本控制工具（如 Git）進行翻譯檔案同步與歷史版本保存。  
   - 定期收集使用者回饋，若有不自然翻譯或錯誤用詞，立即修正。
